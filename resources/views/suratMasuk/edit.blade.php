<!-- resources/views/suratMasuk/edit.blade.php -->
        ...
        <form action="{{ route('suratMasuk.update', $suratMasuk->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <table class="table">
                <tr>
                    ...
                    <td><input type="text" name="nomor_surat" id="nomor_surat" class="form-control" value="{{ old('nomor_surat', $surat->nomor_surat) }}" required></td>
                </tr>
                <tr>
                    <td><label for="pengirim" class="form-label">Pengirim</label></td>
                    <td><input type="text" name="pengirim" id="pengirim" class="form-control" value="{{ old('pengirim', $surat->pengirim) }}" required></td>
                </tr>
                <tr>
                    <td><label for="tanggal" class="form-label">Tanggal</label></td>
                    <td><input type="date" name="tanggal" id="tanggal" class="form-control" value="{{ old('tanggal', $surat->tanggal->format('Y-m-d')) }}" required></td>
                </tr>
                <tr>
                    <td><label for="perihal" class="form-label">Perihal</label></td>
                    <td><input type="text" name="perihal" id="perihal" class="form-control" value="{{ old('perihal', $surat->perihal) }}" required></td>
                </tr>
                <tr>
                    <td><label for="isi" class="form-label">Isi</label></td>
                    <td><textarea name="isi" id="isi" class="form-control" rows="4" required>{{ old('isi', $surat->isi) }}</textarea></td>
                </tr>
                <tr>
                    <td><label for="status" class="form-label">Status</label></td>
                    <td>
                        <select name="status" id="status" class="form-control" required>
                            <option value="baru" {{ (old('status', $surat->status) == 'baru') ? 'selected' : '' }}>Baru</option>
                            <option value="proses" {{ (old('status', $surat->status) == 'proses') ? 'selected' : '' }}>Proses</option>
